...
Workload::Workload(long long int nodes) : _nodes(nodes)
{
}
...
Workload *Workload::New(string const &workload, long long int nodes,
                        Configuration const *const config)
{
  ...
  else if (workload_name == "synthetic")
  {
    ...
    vector<long long int> sizes = (params.size() > 3) ? tokenize_int(params[3]) : vector<long long int>(1, config ? config->GetLongInt("packet_size") : 1);
    vector<long long int> rates = (params.size() > 4) ? tokenize_int(params[4]) : vector<long long int>(1, 1);
  ...
  else if (workload_name == "trace")
  {
    ...
    vector<long long int> packet_sizes = tokenize_int(params[1]);
    ...
    if (params.size() > 2)
    {
      ...
      if (params.size() > 3)
      {
        skip = atoll(params[3].c_str());
        if (params.size() > 4)
        {
          scale = atoll(params[4].c_str());
        }
      }
    }
  ...
  else if (workload_name == "netrace")
  {
    ...
    string const &filename = params[0];
    long long int channel_width = config ? (config->GetLongInt("channel_width")) : 128;
    long long int size_offset = 0;
    ...
    if (params.size() > 1)
    {
      ...
      if (params.size() > 2)
      {
        scale = atoll(params[2].c_str());
        if (params.size() > 3)
        {
          region = atoll(params[3].c_str());
          if (params.size() > 4)
          {
            enforce_deps = atoll(params[4].c_str());
            if (params.size() > 5)
            {
              enforce_lats = atoll(params[5].c_str());
              if (params.size() > 6)
              {
                size_offset = atoll(params[6].c_str());
              }
            }
          }
        }
...
void Workload::advanceTime()
{
  while (!_deferred_nodes.empty())
  {
    long long int const source = _deferred_nodes.front();
    _deferred_nodes.pop();
...
long long int Workload::source() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
...
SyntheticWorkload::SyntheticWorkload(long long int nodes, double load,
...
void SyntheticWorkload::advanceTime()
{
  ...
  {
    long long int const source = _sleeping_nodes.front();
    _sleeping_nodes.pop();
...
long long int SyntheticWorkload::dest() const
{
  //assert(!_pending_nodes.empty());
  long long int const dest = _traffic->dest(source());
  //assert((dest >= 0) && (dest < _nodes));
  return dest;
}

long long int SyntheticWorkload::size() const
{
  //assert(!_pending_nodes.empty());
  long long int num_sizes = _sizes.size();
  if (num_sizes == 1)
  {
    return _sizes[0];
  }
  long long int pct = RandomInt(_max_val);
  for (long long int i = 0; i < (num_sizes - 1); ++i)
  {
...
long long int SyntheticWorkload::time() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
...
void SyntheticWorkload::inject(long long int pid)
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  _pending_nodes.pop();
...
TraceWorkload::TraceWorkload(long long int nodes, string const &filename,
                             vector<long long int> const &packet_sizes,
                             long long int limit, unsigned long long int skip, unsigned long long int scale)
    : Workload(nodes),
...
{
  unsigned long long int time = _time;
  while (((_limit < 0) || (_count < (unsigned long long int)_limit)) && !_trace->eof())
  {
    ++_count;
    long long int delay, source, dest, type;
    *_trace >> delay >> source >> dest >> type;
    //assert(delay >= 0);
    //assert((source >= 0) && (source < _nodes));
    //assert((dest >= 0) && (dest < _nodes));
    time += (unsigned long long int)delay;
    if (type >= 0)
    {
      _next_source = source;
...
{
  Workload::advanceTime();
  for (unsigned long long int s = 0; s < _scale; ++s)
  {
    ++_time;
    if (_next_source >= 0)
    {
      unsigned long long int const time = _next_packet.time;
      //assert(time >= _time);
      if (time == _time)
      {
...
long long int TraceWorkload::dest() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
  //assert(!_ready_packets[source].empty());
  long long int const dest = _ready_packets[source].front().dest;
  //assert((dest >= 0) && (dest < _nodes));
  return dest;
}

long long int TraceWorkload::size() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
  //assert(!_ready_packets[source].empty());
  long long int const size = _packet_sizes[_ready_packets[source].front().type];
  //assert(size > 0);
  return size;
}

long long int TraceWorkload::time() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
  //assert(!_ready_packets[source].empty());
  long long int const time = (long long int)(_ready_packets[source].front().time / _scale);
  //assert(time >= 0);
  return time;
}

void TraceWorkload::inject(long long int pid)
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
...
void TraceWorkload::retire(long long int pid)
{
}
...
NetraceWorkload::NetraceWorkload(long long int nodes, string const &filename,
                                 unsigned long long int channel_width,
                                 long long int limit, unsigned long long int scale,
                                 long long int region, bool enforce_deps,
                                 bool enforce_lats,
                                 unsigned long long int size_offset)
      ...
      _scale(scale), _enforce_deps(enforce_deps), _enforce_lats(enforce_lats)
{
  ...
  unsigned long long int last_region;
  if (region < 0)
  {
    _region = 0;
  ...
  {
    //assert((unsigned long long int)region < header->num_regions);
    _region = (unsigned long long int)region;
    last_region = (unsigned long long int)region;
  }
  for (unsigned long long int r = 0; r < _region; ++r)
  {
...
#endif
  _limit = 0ll;
  for (unsigned long long int r = _region; r <= last_region; ++r)
  {
...
#endif
      long long int const source = _next_packet->src;
      //assert((source >= 0) && (source < _nodes));
...
{
  //glint
  asyncConfig[0]->netraceInterCycle = (asyncConfig[0]->netraceInterCycle + 1) % asyncConfig[0]->traceStretch;

  //glint

  for (set<unsigned long long int>::iterator iter = _check_packets.begin();
       iter != _check_packets.end(); ++iter)
  {
    unsigned long long int id = *iter;
#ifdef DEBUG_NETRACE
    cout << "ADVANC: Checking if packet " << id << " is cleared." << endl;
#endif
    map<unsigned long long int, nt_packet_t *>::iterator piter = _stalled_packets.find(id);
      ...
      if (_enforce_lats)
      {
        ...
        if ((nt_get_src_type(packet) == 1) &&
            ((packet->type == 1) ||
             ...
             (packet->type == 29)))
        {
          long long int const source = packet->src;
          //assert((source >= 0) && (source < _nodes));
          ...
          {
            latency = (unsigned long long int)(packet->cycle - last_response_eject_time);
          }
        }
        else
        {
  ...
  for (unsigned long long int s = 0; s < _scale; ++s)
  {
    if (asyncConfig[0]->netraceInterCycle == 0)
    {
      ++_time;
    }
...
long long int NetraceWorkload::dest() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
  //assert(!_ready_packets[source].empty());
  long long int const dest = _ready_packets[source].front()->dst;
  //assert((dest >= 0) && (dest < _nodes));
  return dest;
}

long long int NetraceWorkload::size() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
  //assert(!_ready_packets[source].empty());
  long long int const size = nt_get_packet_size(_ready_packets[source].front());
  //assert(size > 0);
  long long int const scaled_size = (8 * size + _size_offset + _channel_width - 1) / _channel_width;
  return scaled_size;
}

long long int NetraceWorkload::time() const
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
  //assert(!_ready_packets[source].empty());
  long long int const time = (long long int)(_ready_packets[source].front()->cycle * asyncConfig[0]->traceStretch / _scale);
  //assert(time >= 0);
  return time;
}

void NetraceWorkload::inject(long long int pid)
{
  //assert(!_pending_nodes.empty());
  long long int const source = _pending_nodes.front();
  //assert((source >= 0) && (source < _nodes));
...
void NetraceWorkload::retire(long long int pid)
{
  map<long long int, nt_packet_t *>::iterator iter = _in_flight_packets.find(pid);
  ...
  if (_enforce_deps && _enforce_lats &&
      ...
      ((packet->type == 2) ||
       ...
       (packet->type == 30)))
  {
    map<unsigned long long int, unsigned long long int>::iterator iter = _response_eject_time.find(packet->id);
    //assert(iter != _response_eject_time.end());
    unsigned long long int const eject_time = iter->second;
    //assert(_time >= eject_time - 1);
    long long int const dest = packet->dst;
    //assert((dest >= 0) && (dest < _nodes));
#ifdef DEBUG_NETRACE
  ...
  {
    unsigned long long int const dep = packet->deps[d];
#ifdef DEBUG_NETRACE
