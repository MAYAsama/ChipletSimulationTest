    ...
    : Router(config, parent, name, id, inputs, outputs), _active(false)
{
  ...
  //glint
  _routing_delay = asyncConfig[_netid]->getRoutingDelay(id);
  _vc_alloc_delay = asyncConfig[_netid]->getVcAllocDelay(id);
  ...
  _sw_alloc_delay = asyncConfig[_netid]->getSwAllocDelay(id);
...
{
  //==========================gating===================================
  if (asyncConfig[_netid]->doGating)
  {
    if (asyncConfig[_netid]->gatingMode == 0)
    {
      if (!_active)
      {
        asyncConfig[_netid]->idleTicksCounter[_id]++;
        if (asyncConfig[_netid]->idleTicksCounter[_id] >= asyncConfig[_netid]->sleepThreshold)
        {
          asyncConfig[_netid]->gatedTicksCounter[_id]++;
        }
      }
      else
      {
        if (asyncConfig[_netid]->idleTicksCounter[_id] > 0)
        {

          if (asyncConfig[_netid]->idleTicksCounter[_id] >= asyncConfig[_netid]->breakEvenThreshold)
          {

            asyncConfig[_netid]->viableIdleTicksSum[_id] = asyncConfig[_netid]->viableIdleTicksSum[_id] + asyncConfig[_netid]->idleTicksCounter[_id] - asyncConfig[_netid]->breakEvenThreshold;
            asyncConfig[_netid]->viableIdleTimesSum[_id]++;
          }

          asyncConfig[_netid]->idleTicksCounter[_id] = 0;

          if (asyncConfig[_netid]->gatedTicksCounter[_id] > 0)
          {
            asyncConfig[_netid]->viableGatedTicksSum[_id] = asyncConfig[_netid]->viableGatedTicksSum[_id] + asyncConfig[_netid]->gatedTicksCounter[_id] - asyncConfig[_netid]->breakEvenThreshold;
            asyncConfig[_netid]->gatedTicksCounter[_id] = 0;
            asyncConfig[_netid]->gatedTimesSum[_id]++;
          }
        }
      }
    }
    else if (asyncConfig[_netid]->gatingMode == 1)
    {
      if (!_active)
      {
        ...
        if (asyncConfig[_netid]->idleTicksCounter[_id] >= asyncConfig[_netid]->routerSleepThreshold[_id])
        {
          asyncConfig[_netid]->gatedTicksCounter[_id]++;
        }
      }
      else
      {
        ...
        if (asyncConfig[_netid]->idleTicksCounter[_id] > 0)
        {
          ...
          if (asyncConfig[_netid]->idleTicksCounter[_id] >= asyncConfig[_netid]->breakEvenThreshold)
          {
            ...
            if ((asyncConfig[_netid]->idleTicksCounter[_id] - asyncConfig[_netid]->breakEvenThreshold) < asyncConfig[_netid]->routerSleepThreshold[_id])
            {
              flagSpuriousCase = 1;
            }
          ...
          if (asyncConfig[_netid]->gatedTicksCounter[_id] > 0)
          {
            ...
            if (asyncConfig[_netid]->gatedTicksCounter[_id] > asyncConfig[_netid]->breakEvenThreshold)
            {
              //successfull gating
              if (asyncConfig[_netid]->routerSleepThreshold[_id] <= asyncConfig[_netid]->sleepThresholdStep)
              {
                //do nothing..
              }
              else
              {
                ...
                asyncConfig[_netid]->routerSleepThreshold[_id] = asyncConfig[_netid]->routerSleepThreshold[_id] - asyncConfig[_netid]->sleepThresholdStep;
              }
            }
            else
            { //failed gating
              ...
              {
                 asyncConfig[_netid]->routerSleepThreshold[_id] = asyncConfig[_netid]->sleepThresholdStep;
              }
              else
              {
                if (asyncConfig[_netid]->routerSleepThreshold[_id] >= asyncConfig[_netid]->breakEvenThreshold)
                {
                  //do nothing..
                }
                else
  ...
  asyncConfig[_netid]->queueTicks += _in_queue_flits.size();
  asyncConfig[_netid]->routeTicks += _route_vcs.size();
  asyncConfig[_netid]->vcaTicks += _vc_alloc_vcs.size();
  asyncConfig[_netid]->swaTicks += _sw_alloc_vcs.size();
  asyncConfig[_netid]->crossbarTicks += _crossbar_flits.size();
...
bool IQRouter::_ReceiveFlits()
{
  ...
  for (long long int input = 0; input < _inputs; ++input)
  {
    ...
    if (f)
    {
      ...
      activity = true;
      //      printf("\nTime:,%lld,%lld,[%lld][%lld],ReceiveFlit,%lld\n", GetSimTime(), this->inode+this->routercounts*64, f->id, f->pid, f->vc); //Sneha
      //printf("\nTime:,%lld,%lld,[%lld][%lld],ReceiveFlit,%lld,%lld,%lld", GetSimTime(), this->inode+this->routercounts*64, f->id, f->pid, f->vc,f->src,f->dest); //Sneha
    }
  }
  return activity;
}
...
bool IQRouter::_ReceiveCredits()
{
  ...
  for (long long int output = 0; output < _outputs; ++output)
  {
    ...
    {
      //glint
      _proc_credits.push_back(make_pair(GetSimTime() + asyncConfig[_netid]->getCreditDelay(_id), make_pair(c, output)));
...
void IQRouter::_InputQueuing()
{
  for (map<long long int, Flit *>::const_iterator iter = _in_queue_flits.begin(); iter != _in_queue_flits.end(); ++iter)
  {
    ...
    //printf("\nTime:,%lld,%lld,[%lld][%lld],InputQueueing,%lld,%lld,%lld", GetSimTime(), this->inode+this->routercounts*64, f->id, f->pid, f->vc,f->src,f->dest); //Sneha
...
void IQRouter::_RouteEvaluate()
{
  for (deque<pair<long long int, pair<long long int, long long int>>>::iterator iter = _route_vcs.begin(); iter != _route_vcs.end(); ++iter)
  {
    ...
    iter->first = GetSimTime() + asyncConfig[_netid]->getRoutingDelay(_id) - 1;
  }
}
...
void IQRouter::_VCAllocEvaluate()
{
  ...
  for (deque<pair<long long int, pair<pair<long long int, long long int>, long long int>>>::iterator iter = _vc_alloc_vcs.begin(); iter != _vc_alloc_vcs.end(); ++iter)
  {
    ...
    iter->first = GetSimTime() + asyncConfig[_netid]->getVcAllocDelay(_id) - 1;
    long long int const input = iter->second.first.first;
    long long int const vc = iter->second.first.second;
...
void IQRouter::_VCAllocUpdate()
{
  while (!_vc_alloc_vcs.empty())
  {
    ...
    //printf("\nTime:,%lld,%lld,[%lld][%lld],VCUpdate,%lld,%lld,%lld", GetSimTime(), this->inode+this->routercounts*64, f->id, f->pid, f->vc,f->src,f->dest); //Sneha
  }
}
...
void IQRouter::_SWAllocEvaluate()
{
  ...
  for (deque<pair<long long int, pair<pair<long long int, long long int>, long long int>>>::iterator iter = _sw_alloc_vcs.begin(); iter != _sw_alloc_vcs.end(); ++iter)
  {
    ...
    iter->first = GetSimTime() + asyncConfig[_netid]->getSwAllocDelay(_id, dest) - 1;
...
void IQRouter::_SWAllocUpdate()
{
  while (!_sw_alloc_vcs.empty())
  {
    ...
    //printf("\nTime:,%lld,%lld,[%lld][%lld],SWAllocUpdate,%lld,%lld,%lld", GetSimTime(), this->inode+this->routercounts*64, f->id, f->pid, f->vc ,f->src,f->dest); //Sneha
  }
}
...
void IQRouter::_SwitchEvaluate()
{
  for (deque<pair<long long int, pair<Flit *, pair<long long int, long long int>>>>::iterator iter = _crossbar_flits.begin(); iter != _crossbar_flits.end(); ++iter)
  {
    ...
    iter->first = GetSimTime() + asyncConfig[_netid]->getStFinalDelay(_id) - 1;
...
void IQRouter::_SwitchUpdate()
{
  while (!_crossbar_flits.empty())
  {
    ...
    //printf("\nTime:,%lld,%lld,[%lld][%lld],SWUpdate,%lld,%lld,%lld", GetSimTime(), this->inode+this->routercounts*64, f->id, f->pid, f->vc,f->src,f->dest); //Sneha
...
void IQRouter::_SendFlits()
{
  for (long long int output = 0; output < _outputs; ++output)
  {
    if (!_output_buffer[output].empty())
    {
      ...
      //printf("\nTime:,%lld,%lld,[%lld][%lld],SendingFlit,%lld,%lld,%lld,output port =%lld", GetSimTime(), this->inode+this->routercounts*64, f->id, f->pid, f->vc,f->src,f->dest,output); //Sneha
    }
  }
}
