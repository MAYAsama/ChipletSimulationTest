...
TrafficManager *TrafficManager::New(Configuration const config[], vector<Network *> const &net)
{
  ...
  string sim_type = config[0].GetStr("sim_type");
...
{
  _totalnode=0;
  for(long long int i=0;i<net.size();i++)
  {
	_totalnode+=_net[i]->NumNodes();
  }	
  _nodes = _totalnode;
  //_nodes = _net[0]->NumNodes();
  _routers = _net[0]->NumRouters();

  _vcs = config[0].GetLongInt("num_vcs");
  _subnets = config[0].GetLongInt("subnets");

  timeStretch = config[0].GetLongInt("traceStretch");

  ticksTo1ns=config[0].GetLongInt("ticksTo1ns");
  ...
  string priority = config[0].GetStr("priority");
  ...
  vector<string> rf(net.size());
  _rf.resize(net.size());
  for(int i=0;i<net.size();i++)
  {
    rf[i]=config[i].GetStr("routing_function") + "_" + config[i].GetStr("topology");
    map<string, tRoutingFunction>::const_iterator rf_iter = gRoutingFunctionMap.find(rf[i]);
    if (rf_iter == gRoutingFunctionMap.end())
    {
      Error("Invalid routing function: " + rf[i]);
    }
    _rf[i] = rf_iter->second;
  }
  
/*
  string rf = config[0].GetStr("routing_function") + "_" + config[0].GetStr("topology");
  ...
  _rf = rf_iter->second;
*/
  _lookahead_routing = !config[0].GetLongInt("routing_delay");
  _noq = config[0].GetLongInt("noq");
  if (_noq)
  {
  ...
  _classes = config[0].GetLongInt("classes");

  _subnet = config[0].GetIntArray("subnet");
  if (_subnet.empty())
  {
    _subnet.push_back(config[0].GetLongInt("subnet"));
  }
  _subnet.resize(_classes, _subnet.back());

  _class_priority = config[0].GetIntArray("class_priority");
  if (_class_priority.empty())
  {
    _class_priority.push_back(config[0].GetLongInt("class_priority"));
  }
  ...
  for (long long int source = 0; source < _nodes; ++source)
  {
    ...
    for (long long int subnet = 0; subnet < _subnets; ++subnet)
    {
      ...
      BufferState *bs = new BufferState(config[0], this, tmp_name.str());
      long long int vc_alloc_delay = config[0].GetLongInt("vc_alloc_delay");
      long long int sw_alloc_delay = config[0].GetLongInt("sw_alloc_delay");
      long long int router_latency = config[0].GetLongInt("routing_delay") + (config[0].GetLongInt("speculative") ? max(vc_alloc_delay, sw_alloc_delay) : (vc_alloc_delay + sw_alloc_delay));
      long long int actualnode=source;
      long long int chipnum=_Nodeshift(source);
      for(long long int i=0;i<chipnum;i++)
      {
        actualnode-=_net[i]->NumNodes();
      }
      long long int min_latency = 1 + _net[chipnum]->GetInject(actualnode)->GetLatency() + router_latency + _net[chipnum]->GetInjectCred(actualnode)->GetLatency();
  ...
  _hold_switch_for_packet = config[0].GetLongInt("hold_switch_for_packet");

  // ============ Simulation parameters ============

  _total_sims = config[0].GetLongInt("sim_count");
  ...
  {
    seed = config[0].GetLongInt("seed");
  }
  RandomSeed(seed);

  _measure_stats = config[0].GetIntArray("measure_stats");
  if (_measure_stats.empty())
  {
    _measure_stats.push_back(config[0].GetLongInt("measure_stats"));
  }
  _measure_stats.resize(_classes, _measure_stats.back());
  _pair_stats = (config[0].GetLongInt("pair_stats") == 1);

  _include_queuing = config[0].GetLongInt("include_queuing");

  _print_csv_results = config[0].GetLongInt("print_csv_results");
  _deadlock_warn_timeout = config[0].GetLongInt("deadlock_warn_timeout");

  string watch_file = config[0].GetStr("watch_file");
  ...
  // Orion Power Support
  string orion_file = config[0].GetStr("orion_file");
  if ((orion_file != "") && (orion_file != "-"))
  {
    _orion_file = orion_file;
  }

  vector<long long int> watch_flits = config[0].GetIntArray("watch_flits");
  for (size_t i = 0; i < watch_flits.size(); ++i)
  {
    _flits_to_watch.insert(watch_flits[i]);
  }

  vector<long long int> watch_packets = config[0].GetIntArray("watch_packets");
  for (size_t i = 0; i < watch_packets.size(); ++i)
  {
    _packets_to_watch.insert(watch_packets[i]);
  }

  string stats_out_file = config[0].GetStr("stats_out");
  if (stats_out_file == "")
  {
    _stats_out = NULL;
  }
  ...
  else
  {
    ...
    config[0].WriteMatlabFile(_stats_out);
  }

  // Orion Power Support
  string orion_out_file = config[0].GetStr("orion_out");
  if (orion_out_file == "")
  {
    _orion_out = NULL;
  }
  ...
  else
  {
    ...
    config[0].WriteMatlabFile(_orion_out);
  }
...
void TrafficManager::Setdeque25(deque<TimedModule*> mo)
{
    _25timed_modules=mo;
}

void TrafficManager::Setleftnode(vector<long long int> mo)
{
    leftnode=mo;
}
long long int TrafficManager::_Nodeshift(long long int globalnode)    //用于确认节点所在拓扑
{
    long long int i = 0;
    long long int nodesum = _net[0]->NumNodes();
    while (globalnode >= nodesum) 
    {
        i += 1;
        nodesum += _net[i]->NumNodes();
    }
    return i;
}
...
long long int TrafficManager::_GeneratePacket(long long int source, long long int dest, long long int size, long long int cl, long long int time)
{
  ...
  for (long long int i = 0; i < size; ++i)
  {
    ...
    auto index1=0;
    auto index2=0;
    long long int chipnum1=0;
    long long int chipnum2=0;
    if(_net.size()>1)
    {
    f->isrcnode=f->src-_net[0]->NumNodes();
    if(f->dest >= _net[0]->NumNodes())
    	f->idesnode=f->dest-_net[0]->NumNodes();
    else
    {
    	f->idesnode=f->dest;
    }
    vector<long long int>::iterator it1 = find(leftnode.begin(), leftnode.end(), f->isrcnode);
    index1 = std::distance(std::begin(leftnode), it1);
    chipnum1=_Nodeshift(index1+_net[0]->NumNodes());
    vector<long long int>::iterator it2 = find(leftnode.begin(), leftnode.end(), f->idesnode);
    index2 = std::distance(std::begin(leftnode), it2);
    chipnum2=_Nodeshift(index2+_net[0]->NumNodes());
    }
    else
    {
      f->isrcnode=f->src;
    ...
    /*if(f->dest>_net[0]->NumNodes()-1)
    {
      f->ipnodes=_net[0]->NumNodes();
    }
    else
    {
      f->ipnodes=0;
    }
      int actualnode1=f->src;
      int chipnum1=_Nodeshift(f->src);
      for(int i=0;i<chipnum1;i++)
      {
        actualnode1-=_net[i]->NumNodes();
      }
      int actualnode2=f->dest;
      int chipnum2=_Nodeshift(f->dest);
      for(int i=0;i<chipnum2;i++)
      {
        actualnode2-=_net[i]->NumNodes();
      }
      f->isrcnode=_net[chipnum1]->_routers[actualnode1]->inode ;
      f->idesnode=_net[chipnum2]->_routers[actualnode2]->inode ;*/

    if(f->dest >= _net[0]->NumNodes())
    f->inbondnode = _bondnode[chipnum2-1][mp[index2%16]];
    else
    f->inbondnode = _bondnode[0][mp[index2%16]];
   
    //f->inbondnode = f->idesnode;
    
    if(_net.size()>1)    //f->viaint==cnum
    {
    	if(f->dest<_net[0]->NumNodes())
    	f->viaint = 0;//
    	else 
    ...
    {
      _partial_packets[cl][index1+_net[0]->NumNodes()].push_back(f);
    }
  }
  return pid;
}
...
void TrafficManager::_Step()
{
  ...
  for (long long int subnet = 0; subnet < _subnets; ++subnet)
  {
    ...
    {
//gyj
      long long int actualnode=n;
      long long int chipnum=_Nodeshift(n);
      for(long long int i=0;i<chipnum;i++)
      {
        actualnode-=_net[i]->NumNodes();
      }
      Flit * const f = _net[chipnum]->ReadFlit( actualnode );
      if (f)
      {
      ...
      Credit * const c = _net[chipnum]->ReadCredit( actualnode );
      if (c)
      {
    ...
    for(long long int i=0;i<_net.size();i++)
    {
      _net[i]->ReadInputs();
    }
//
    for (deque<TimedModule*>::const_iterator iter = _25timed_modules.begin();
        iter != _25timed_modules.end();
        ++iter) {
        (*iter)->ReadInputs();
    }
  }
    ...
    for (long long int n = 0; n < _nodes; ++n)
    {
      ...
      for (long long int i = 1; i <= class_limit; ++i)
      {
        ...
        { // Find first available VC
          OutputSet route_set;
          _rf[chipnum](NULL, cf, -1, &route_set, true);
          ...
          if (_noq)
          {
            ...
            cf->vc = vcBegin;
            _rf[chipnum](router, cf, in_channel, &cf->la_route_set, false);
            cf->vc = -1;
        ...
        if (f->head)
        {
          if (_lookahead_routing)
          {
            if (!_noq)
            {
              ...
              const Router *router = inject->GetSink();
              long long int in_channel = inject->GetSinkPort();
              _rf[chipnum](router, f, in_channel, &f->la_route_set, false);
            }
            else if (f->watch)
            {
...
//gyj
	...
	_net[chipnum]->WriteFlit(f, actualnode);//
      }
    }
  }
  ...
  for (long long int subnet = 0; subnet < _subnets; ++subnet)
  {
	...
	_net[chipnum]->WriteCredit(c, actualnode);
...
//gyj
    ...
    for (long long int chipnum = 0; chipnum < _net.size(); chipnum++)
    {
        _net[chipnum]->Evaluate();
        _net[chipnum]->WriteOutputs();

    }
    
    for (deque<TimedModule*>::const_iterator iter = _25timed_modules.begin();//对2.5D的channel中的数据进行处理
        iter != _25timed_modules.end();
        ++iter) {
        (*iter)->Evaluate();
        (*iter)->WriteOutputs();
...
void TrafficManager::_DisplayOverallClassStats(long long int c, ostream &os) const
{
  ...
  //cout<<"FlitTimeBreakup,IQ,RC,VC,SA,ST,"<<asyncConfig[0]->queueTicks/Generated_flits <<","<<asyncConfig[0]->routeTicks/Generated_flits <<","<<asyncConfig[0]->vcaTicks/Generated_flits <<","<<asyncConfig[0]->swaTicks/Generated_flits <<","<<asyncConfig[0]->crossbarTicks/Generated_flits <<endl;
