...
public:
  ...
  void Setdeque25(deque<TimedModule*> mo);//
  vector<long long int> leftnode;
  void Setleftnode(vector<long long int> mo);
//  static TrafficManager *New(Configuration const &config,
//                             vector<Network *> const &net);
  static TrafficManager * New(const Configuration  config[], vector<Network *> const & net);

  bool Run();
//
  long long int _Nodeshift(long long int globalnode);
  void UpdateStats();
